...
def train(data_dict, hyp, opt, device, tb_writer=None):
...
def train_main(paramater=None):
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        opt.save_dir = increment_path(Path(__file__).parent/Path(opt.project) / opt.name, exist_ok=opt.exist_ok | opt.evolve)  # increment run
    ...
    if not opt.evolve:
        ...
        if paramater is None:
            train(None, hyp, opt, device, tb_writer)
        else:
            return hyp, opt, device, tb_writer, opt.save_dir
